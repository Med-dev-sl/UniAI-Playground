...
const Auth = () => {
  ...
  // After signup, move to chat if they have a course, else go to home to pick one
  useEffect(() => {
    if (user && !isLogin) {
      if (courseState.courseId) {
        ...
        const autoComplete = async () => {
          try {
            await new Promise(resolve => setTimeout(resolve, 800)); // wait for trigger
            ...
            navigate('/'); // Fallback to landing if failure
          }
        };
        autoComplete();
      } else {
  ...
  }, [user, isLogin, courseState, navigate, toast]);
      ...
      if (isLogin) {
        ...
        if (profile?.course_id && profile?.faculty_id && profile?.academic_level) {
          ...
          if (courseState.courseId) {
            ...
            // Need to complete registration manually, send to landing
            navigate('/');
          }
        }
      } else {
